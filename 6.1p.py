...
class Lane_Detector:
    ...
    def image_callback(self, msg):
        ...
        white_lines = self.hough_transformation(edges_white)
        yellow_lines = self.hough_transformation(edges_yellow)

        # Draw lines found on both Hough Transforms on the cropped image
        self.lines_draw(cropped_img, white_lines)
        self.lines_draw(cropped_img, yellow_lines)

        # Display the image with detected lines
        cv2.imshow('yellow filter', yellow_mask)
        cv2.imshow('white filter', white_mask)
        cv2.waitKey(1)

    def hough_transformation(self, img):
        # Apply Hough Transform
        lines = cv2.HoughLinesP(img, rho=1, theta=np.pi/180, threshold=100, minLineLength=50, maxLineGap=50)
        return lines

    def lines_draw(self, img, lines):
